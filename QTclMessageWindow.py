...
class QTclMessageWindow(QTclPopupWindow):
    ...
    def __init__(self, parent, title, user_title, user_message, exc=None):
        """initialize the message box with a title, a user title,
        a username and optional exception the messagebox transforms into an
        error message box.
        """
        ...
        super().__init__(parent, title, is_modal=True)

    def make_gui(self, title):
        self.setwintitle(title)

        row = 0
        titlab = self.makelabel(
            caption=self.user_title,
            sticky=tk.W,
            cspan=2,
            padx=(5, 5),
            pady=(5, 5)
        ...
        if self.exc != None:
            self.bu_row = row
            self.det_bu = self.makebutton(
                erow=row,
                ecol=1,
                caption="show details",
                cmd=self.show_details,
                sticky=tk.E
                )
            row += 2
