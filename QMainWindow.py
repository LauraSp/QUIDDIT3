...
class MainWindow(QTclBaseWindow):
    ...
    def make_gui(self, title):
        ...
        self.main_canvas = self.make_mplcanvas(
            fig=self.main_fig, erow=row, ecol=0, cspan=4
        )
        self.message = tk.Text(self, state='disabled', relief=STDRELIEF)
        self.message.grid(
            row=row, column=4, columnspan=2, sticky=tk.NSEW, padx=5
        )
        ...
        self.back_button = self.makebutton(
            erow=row, ecol=0, caption='Previous',
            cmd=self.display_prev,
            state=tk.DISABLED, padx=5, pady=5, sticky=tk.W
        )
        self.next_button = self.makebutton(
            erow=row, ecol=3, caption='Next',
            cmd=self.display_next,
            state=tk.DISABLED, padx=5, pady=5, sticky=tk.E
        )

        self.histo_button = self.makebutton(
            erow=row, ecol=2, caption='Histogram',
    ...
    def print_result(self, header, result):
        ...
        for idx, item in enumerate(zip(names, result)):
            if idx == 0:
                self.print_message(
                    self.message,
                    'results for spectrum {}'.format(
                        str(result['name']).split('/')[-1])
                )
            else:
                self.print_message(
                    self.message,
                    '{}: {}'.format(item[0], np.round(item[1], 2))
                    )
    ...
    def create_menu_bar(self):
        ...
        revmenuopt = {
            'Review General Deconvolution': self.review_deconv,
        ...
        helpmenuopt = {
            'GitHub': (lambda s=self: webbrowser.open(QSettings.github_url)),
            'About': self.about
            }
    ...
    def convert_ENVI(self):
        try:
            enviwindow = QENVIconversionWindow(
                self, "ENVI Conversion", self.convenvidta
                )
    ...
    def convert_DPT(self):
        try:
            dptwindow = QDPTConversionWindow(
                self, "dpt Converison", self.convdptdta
                )

            if dptwindow.dresult == 'OK':
                self.convdptdta = QDPTconvinp(
                    dptwindow.dpt_file, dptwindow.target_dir
                    )
    ...
    def click_exit(self):
        try:
            mwin = QTclMessageWindow(
                mw,
                "QUIDDIT Question",
                "Just to make sure ...",
                "Do you really want to quit QUIDDIT?"
                )
            if mwin.dresult == 'OK':
                self.root.destroy()
            ...
            QTclMessageWindow(
                mw,
                "QUIDDIT Error", "An unhandled error has occured",
                "Original message: {}".format(str(e)),
                e
                )
    ...
    def plot_ls(self):
        try:
            resfile_window = QAskFileWindow(
                self,
                "File Selection",
                'Select linescan results',
                self.lineresultfile
            )
    ...
    def plot_map(self):
        try:
            resfile_window = QPlotMapInpWindow(
                self,
                "Input for map plotting",
                self.mapinp
                )
            ...
            if resfile_window.dresult == 'OK':
                ...
                self.print_message(
                    self.message,
                    'Plotting map. This may take a few seconds...'
                )
                ch.display_first()
                self.set_can_helper(ch)
    ...
    def plot_batchpeakmap(self):
        try:
            resfile_window = QPlotBatchPeakMapInpWindow(
                self, "Input for map plotting", self.batchpeakmapinp
                )
            ...
            ch.add_map_file(
                self.batchpeakmapinp.map_file, self.batchpeakmapinp.clims
                )
            self.print_message(
                self.message, 'Plotting map. This may take a few seconds...'
            )
            ch.display_first()
            self.set_can_helper(ch)
    ...
    def plot_ENVI(self):
        try:
            envifiles_window = QAskENVIWindow(
                self, "File Selection", self.plotenvidata
                )
    ...
    def process_data(self):
        ...
        try:
            inpf = QProcessDtaInpWindow(
                self, "Deconvolution Input", self.prcdta
                )
            ...
            if inpf.dresult == 'OK':
                ...
                if resultfile == reviewfile:
                    ...
                    self.print_message(
                        self.message,
                        'The same name was entered for result and review file.'
                ...
                with open(resultfile, 'w') as res_fob:
                    res_fob.write(
                        'Results for sample %s - age: %.0f Ma'%(str(samplename),round(age, 3)) + ':\n')
                ...
                for filename in specfiles:
                    ...
                    self.print_message(
                        self.message,
                        'Processing file {}/{}'.format(i, len(specfiles))
                    )
                    result, review = decon.deconvolution(
                        filename, self.prcdta.age, QSettings.N_comp
                        )
    ...
    def peak_fit(self):
        try:
            ...
            if peakinp.dresult =='OK':
                ...
                with open(resultfile, 'w') as res_fob:
                    res_fob.write(
                        'Results for sample ' + str(samplename) + ':\n'
                    )
    ...
    def diamondtype(self):
        try:
            diatype_window = QDiamondTypeWindow(
                self, "Diamond Type Determination", self.diatypedta
                )
            if diatype_window.dresult == 'OK':
                ...
                resultfile = (
                    QSettings.userhome + '\\' + self.diatypedta.result + '.csv'
                    )
                with open(resultfile, 'w') as res_fob:
                ...
                for filename in spectra:
                    ch.add_spectra_files([filename])
                    ch.display_first()
                    diamondtype, warn = diatp.diamondtype(
                        filename, save, outpath=outpath
                        )
                    note = 'ok' if warn == '' else warn
                    self.print_message(
                        self.message,
                        'Processing file {}/{}: {}'.format(
                            i, len(spectra), filename)
                    )
                    self.print_message(
                        self.message, 'type: {}\nnote: {}\n'.format(
                            diamondtype, note)
